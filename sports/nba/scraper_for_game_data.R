...
df_scrape_task <- read_csv(FILENAME_GAMEURLS, col_types = cols()) %>% filter(date < Sys.Date() & !is.na(url))

if (file.exists(FILENAME_GAMELOGS)) {
  df_scrape_task <- df_scrape_task %>% filter(!game.id %in% (read_csv(FILENAME_GAMELOGS, col_types = cols()) %>% .$game.id %>% unique()) )
}

#df_scrape_task <- df_scrape_task %>% sample_n(100)

if (nrow(df_scrape_task) > 0) {
  print("Game data: scrape started")
  ...
  print("Game data: scrape finished")
  
} else {
  print("Game data: Nothing to scrape")
}