#setwd("/Users/gianidepalma/Desktop/NCAAF")
...
stats_tr <- read.csv('stats_ncaaf.csv', stringsAsFactors = FALSE) %>%
  mutate(Stat_tr = gsub('https://www.teamrankings.com/college-football/stat/(.+)',
                        '\\1', x),
...
if(!file.exists('teamrankings_success_dates.csv')){
  success_dates <- c()
  list_date <- seq(from = as.Date('2019-01-01', format = '%Y-%m-%d'),
                   to = Sys.Date(), 
                   by = 'day')
} else {
...
if(length(dw_list) > 0){
  ...
  if(!file.exists('teamrankings_db.csv')){
    ...
    df_tr %>% write.table(file = 'teamrankings_db.csv',
