...
game_urls <- read_csv("game_urls.csv")
...
game_logs2 <- game_logs2 %>% 
  ...
  mutate(season = case_when(
    is.na(season) & month(date) > 9 ~ year(date),
    is.na(season) & month(date) < 6 ~ year(date) - 1,
    TRUE ~ season
    )) %>% 
  assert(not_na, season) %>% 
  verify(nrow(.) == nrow(game_logs2)) %>% 
  select(key, game_id, date, season, Location, everything())
...
game_urls2 <- game_urls2 %>% 
  mutate(game_id = if_else(is.na(game_id), 
                           paste(date, str_sub(season, 3, 4), home_team_id, sep = "-"), 
                           game_id))

z1 <- game_urls2 %>%
  select(key:date, team_id = home_team_id) %>% 
  mutate(homeOrAway = "home",
         key = paste(game_id, homeOrAway, sep = "-"))

z2 <- game_urls2 %>%
  select(key:date, team_id = away_team_id) %>% 
...
if (any_future_games_in_schedule) {
  ...
  next_match_id <- game_urls_long %>% 
    semi_join(next_match_dates, by = c("team_id", "date")) %>% 
    distinct(game_id)
...
z_home <- game_logs2 %>% 
  ...
  rename_at(vars(First_Half_Score:DRtg), ~str_c("home_", .))

z_away <- game_logs2 %>% 
  filter(homeOrAway == "away") %>% 
  rename_at(vars(First_Half_Score:DRtg), ~str_c("away_", .))


z_home2 <- z_home %>% 
  left_join(select(z_away, game_id, team_id_opp = team_id, 
                   starts_with("away_")), 
            by = "game_id") %>% 
  ...
  mutate(at_home = 1L,
         on_the_road= 0L) %>% 
  select(-(opp_First_Half_Score:opp_reserves_PTS), opp_Total_Score)
  
z_away2 <- z_away %>% 
  left_join(select(z_home, game_id, team_id_opp = team_id, 
                   starts_with("home_")), 
            by = "game_id") %>% 
  verify(nrow(.) == nrow(z_home)) %>% 
...
z_sos <- z_long %>% 
  mutate(win = if_else(Total_Score > opp_Total_Score, 1L, 0L),
         loss = if_else(Total_Score < opp_Total_Score, 1L, 0L)) %>% 
  arrange(season, team_id, date) %>%
  group_by(season, team_id) %>% 
  mutate(win2 = cumsum(win),
         loss2 = cumsum(loss)) %>% 
  ungroup() %>% 
  arrange(season, team_id, desc(date)) %>%  # desc(date) important for optimisations
  select(key, season, team_id, date, win2, loss2, team_id_opp) %>% 
  assert(not_na, season)


current_season <- max(z_sos$season)


# Run for past seasons only if not run before
# Load results if past seasons have already been run before
if (file.exists("sos_prev.rds")) {
  sos_df <- readRDS("sos_prev.rds")
  sos_df <- sos_df %>%
    semi_join(z_long, "key") %>% 
    full_join(distinct(z_long, key, season), 
              by = c("key", "season")) %>% 
    mutate(complete = if_else(is.na(complete), 0L, complete)) %>% 
    verify(sum(is.na(.$opp_win_perc)) / nrow(.) < 0.5) %>% 
    verify(sum(is.na(.$opp_opp_win_perc)) / nrow(.) < 0.5)
} else {
  sos_df <- tibble(key = z_sos$key,
                   season = z_sos$season,
                   opp_win_perc = NA_real_,
                   opp_opp_win_perc = NA_real_,
                   complete = 0L)
}

past_seasons <- setdiff(unique(z_sos$season), current_season)
past_seasons_incomplete <- unique(sos_df$season[sos_df$season < current_season & sos_df$complete == 0L])
past_seasons_incomplete

if (length(past_seasons_incomplete) > 0) {
  z_key <- sos_df$key[sos_df$season %in% past_seasons_incomplete]
  
  z_cores <- detectCores()
  z_cl <- makeCluster(z_cores[1] - 1) # not to overload your computer
  registerDoSNOW(z_cl)
  
  z_pb <- txtProgressBar(max = length(z_key), style = 3)
  z_progress <- function(n) setTxtProgressBar(z_pb, n)
  z_opts <- list(progress = z_progress)
  
  z_temp <- foreach (i = 1:length(z_key), .combine = c, .packages = "tidyverse", .options.snow = z_opts) %dopar% {
    index_long <- which(z_sos$key == z_key[i])
    
    return(opponents_record(z_sos, 
                            z_sos$team_id[index_long], 
                            z_sos$date[index_long], 
                            z_sos$season[index_long]))
  }
  
  z_temp2 <- foreach (i = 1:length(z_key), .combine = c, .packages = "tidyverse", .options.snow = z_opts) %dopar% {
    index_long <- which(z_sos$key == z_key[i])
  ...
  z_index <- which(sos_df$key %in% z_key)
  length(z_index) == length(z_temp)
  length(z_index) == length(z_temp2)
  
  sos_df$opp_win_perc[z_index] <- z_temp
  sos_df$opp_opp_win_perc[z_index] <- z_temp2
  sos_df$complete[z_index] <- 1L
  
  if (sum(is.na(sos_df$opp_win_perc)) / nrow(sos_df) < 0.5 && 
      sum(is.na(sos_df$opp_opp_win_perc)) / nrow(sos_df) < 0.5) {
    saveRDS(sos_df, "sos_prev.rds")
  } else {
    warning("Did not save: too much missing data, please check the data source")
  }
}


# Run for current season
z_key <- sos_df$key[sos_df$season %in% current_season]
...
z_long <- z_long %>% 
  left_join(sos_df, c("key", "season")) %>% 
  mutate(sos = (2 * opp_win_perc + opp_opp_win_perc) / 3) %>% 
  select(-complete)
...
z_long2 <- z_long %>%
  ...
  mutate_at(vars(First_Half_Score, Second_Half_Score, OT_score),
            list(avg = cummean_lagged)) %>%
...
df_tz <- df_tz %>% 
  ...
  select(School:Western, Pacific, everything())
...
df_conf <- df_tz %>% 
  select(School) %>% 
  mutate(conf = apply(df_tz[, -(2:6)], 1, function(x) names(which(x == 1))[1]),
         conf = if_else(is.na(conf), "Division2", conf))
...
z_long2 <- z_long2 %>% 
  left_join(df_tz2, by = c("team_id" = "School")) %>% 
  left_join(df_conf, by = c("team_id" = "School")) %>% 
  rename(tz_team = tz) %>%
  left_join(df_tz2, by = c("team_id_opp" = "School")) %>% 
  rename(tz_opp = tz) %>% 
...
map(z_past, range, na.rm = TRUE) # starters, reserves - not sure if something is wrong but both definitely being divided by team Total MP PG
...
# Check stats have been calculated correctly using Michigan States matches
# z_long3 %>% 
#   filter(team_id == "michigan-state") %>% 
#   View()
# 
# z_long3 %>% 
#   filter(team_id == "michigan-state") %>% 
#   writexl::write_xlsx("michigan_state2020.xlsx", format_headers = FALSE)
...
z_home3 <- z_long3 %>% 
  filter(homeOrAway == "home") %>% 
  rename_at(vars(team_id, First_Half_Score:starters_SLR_trend_last5), ~str_c("home_", .x)) %>% 
  select(-homeOrAway, -key)

z_away3 <- z_long3 %>% 
  filter(homeOrAway == "away") %>% 
  rename_at(vars(team_id, First_Half_Score:starters_SLR_trend_last5), ~str_c("away_", .x)) %>% 
  select(-homeOrAway, -key, -Location)
...
z <- df_vf %>%
  arrange(season, date) %>% 
  select(season, date, home_team_id, away_team_id, home_Total_Score, away_Total_Score) %>% 
  rowid_to_column("id") # Add id variable as weighted function currently uses this

z_home <- z %>%
  select(id, date, season,
         team = home_team_id,
         team_opponent = away_team_id,
...
team_un <- c(df_vf$home_team_id, df_vf$away_team_id) %>%
  unique() %>%
  sort()
...
df_vf <- df_vf %>%
  left_join(df_days_off,
            by = c("date" = "Home_date", "home_team_id" = "Home_Team")) %>%
  verify(nrow(.) == nrow(df_vf))

colnames(df_days_off) <- gsub("Home_", "Away_", colnames(df_days_off))
df_vf <- df_vf %>%
  left_join(df_days_off,
            by = c("date" = "Away_date", "away_team_id" = "Away_Team")) %>%
  verify(nrow(.) == nrow(df_vf))
...
calculate_days_on_road <- function(df, team) {
  df_aux <- df %>%
    select(date, home_team_id, away_team_id) %>%
    filter(home_team_id == team | away_team_id == team) %>%
    mutate(TeamTag = ifelse(away_team_id == team, "A", "H"),
           Team = team) %>%
...
df_vf <- select(df_vf, -home_Pace,	-home_starters_MP,-home_starters_FG,	-home_starters_FGA,	-home_starters_2P,	-home_starters_2PA,	-home_starters_3P, -home_starters_3PA,	-home_starters_FT,	
                ...
                -home_opp_ORtg,	-home_opp_DRtg,	-home_at_home,	-home_on_the_road,	-home_opp_reserves_Played, -home_team_id_opp, -home_opp_win_perc, -home_opp_opp_win_perc, -home_opp_Total_Score)
...
df_vf <- select(df_vf, -away_Pace,	-away_starters_MP,-away_starters_FG,	-away_starters_FGA,	-away_starters_2P,	-away_starters_2PA,	-away_starters_3P, -away_starters_3PA,	-away_starters_FT,	
                ...
                -away_opp_ORtg,	-away_opp_DRtg,	-away_at_home,	-away_on_the_road,	-away_opp_reserves_Played, -away_team_id_opp, -away_opp_win_perc, -away_opp_opp_win_perc, -away_opp_Total_Score)

df_vf <- select(df_vf,-game_id,
                -Location,
                -home_starters_MP_trend_last1,
                -home_starters_FG_trend_last1,
                -home_starters_FGA_trend_last1,
                -home_starters_2P_trend_last1,
                -home_starters_2PA_trend_last1,
                -home_starters_3P_trend_last1,
                -home_starters_3PA_trend_last1,
                -home_starters_FT_trend_last1,
                -home_starters_FTA_trend_last1,
                -home_starters_ORB_trend_last1,
                -home_starters_DRB_trend_last1,
                -home_starters_TRB_trend_last1,
                -home_starters_AST_trend_last1,
                -home_starters_STL_trend_last1,
                -home_starters_BLK_trend_last1,
                -home_starters_TOV_trend_last1,
                -home_starters_PF_trend_last1,
                -home_starters_PTS_trend_last1,
                -home_reserves_MP_trend_last1,
                -home_reserves_FG_trend_last1,
                -home_reserves_FGA_trend_last1,
                -home_reserves_2P_trend_last1,
                -home_reserves_2PA_trend_last1,
                -home_reserves_3P_trend_last1,
                -home_reserves_3PA_trend_last1,
                -home_reserves_FT_trend_last1,
                -home_reserves_FTA_trend_last1,
                -home_reserves_ORB_trend_last1,
                -home_reserves_DRB_trend_last1,
                -home_reserves_TRB_trend_last1,
                -home_reserves_AST_trend_last1,
                -home_reserves_STL_trend_last1,
                -home_reserves_BLK_trend_last1,
                -home_reserves_TOV_trend_last1,
                -home_reserves_PF_trend_last1,
                -home_reserves_PTS_trend_last1,
                -home_team_Total_MP_trend_last1,
                -home_team_Total_FG_trend_last1,
                -home_team_Total_FGA_trend_last1,
                -home_team_Total_2P_trend_last1,
                -home_team_Total_2PA_trend_last1,
                -home_team_Total_3P_trend_last1,
                -home_team_Total_3PA_trend_last1,
                -home_team_Total_FT_trend_last1,
                -home_team_Total_FTA_trend_last1,
                -home_team_Total_ORB_trend_last1,
                -home_team_Total_DRB_trend_last1,
                -home_team_Total_TRB_trend_last1,
                -home_team_Total_AST_trend_last1,
                -home_team_Total_STL_trend_last1,
                -home_team_Total_BLK_trend_last1,
                -home_team_Total_TOV_trend_last1,
                -home_team_Total_PF_trend_last1,
                -home_team_Total_PTS_trend_last1,
                -home_opp_team_Total_MP_trend_last1,
                -home_opp_team_Total_FG_trend_last1,
                -home_opp_team_Total_FGA_trend_last1,
                -home_opp_team_Total_2P_trend_last1,
                -home_opp_team_Total_2PA_trend_last1,
                -home_opp_team_Total_3P_trend_last1,
                -home_opp_team_Total_3PA_trend_last1,
                -home_opp_team_Total_FT_trend_last1,
                -home_opp_team_Total_FTA_trend_last1,
                -home_opp_team_Total_ORB_trend_last1,
                -home_opp_team_Total_DRB_trend_last1,
                -home_opp_team_Total_TRB_trend_last1,
                -home_opp_team_Total_AST_trend_last1,
                -home_opp_team_Total_STL_trend_last1,
                -home_opp_team_Total_BLK_trend_last1,
                -home_opp_team_Total_TOV_trend_last1,
                -home_opp_team_Total_PF_trend_last1,
                -home_opp_team_Total_PTS_trend_last1,
                -home_reserves_Played_trend_last1,
                -home_opp_reserves_Played_trend_last1,
                -home_Pace_trend_last1,
                -home_team_Total_FG_pct_trend_last1,
                -home_team_Total_2P_pct_trend_last1,
                -home_team_Total_3P_pct_trend_last1,
                -home_team_TotalFT_pct_trend_last1,
                -home_team_Total_TmPoss_trend_last1,
                -home_opp_team_Total_TmPoss_trend_last1,
                -home_team_Total_ORATING_trend_last1,
                -home_team_Total_DRATING_trend_last1,
                -home_team_Total_EFG_pct_trend_last1,
                -home_team_Total_TS_pct_trend_last1,
                -home_team_Total_ORB_pct_trend_last1,
                -home_team_Total_DRB_pct_trend_last1,
                -home_team_Total_TRB_pct_trend_last1,
                -home_team_Total_HAST_pct_trend_last1,
                -home_team_Total_PAST_pct_trend_last1,
                -home_team_Total_AST_pct_trend_last1,
                -home_team_Total_STL_pct_trend_last1,
                -home_team_Total_BLK_pct_trend_last1,
                -home_team_Total_TOV_pct_trend_last1,
                -home_team_Total_AST_TO_trend_last1,
                -home_team_Total_FTr_trend_last1,
                -home_reserves_BDR_trend_last1,
                -home_starters_SLR_trend_last1,
                -away_starters_MP_trend_last1,
                -away_starters_FG_trend_last1,
                -away_starters_FGA_trend_last1,
                -away_starters_2P_trend_last1,
                -away_starters_2PA_trend_last1,
                -away_starters_3P_trend_last1,
                -away_starters_3PA_trend_last1,
                -away_starters_FT_trend_last1,
                -away_starters_FTA_trend_last1,
                -away_starters_ORB_trend_last1,
                -away_starters_DRB_trend_last1,
                -away_starters_TRB_trend_last1,
                -away_starters_AST_trend_last1,
                -away_starters_STL_trend_last1,
                -away_starters_BLK_trend_last1,
                -away_starters_TOV_trend_last1,
                -away_starters_PF_trend_last1,
                -away_starters_PTS_trend_last1,
                -away_reserves_MP_trend_last1,
                -away_reserves_FG_trend_last1,
                -away_reserves_FGA_trend_last1,
                -away_reserves_2P_trend_last1,
                -away_reserves_2PA_trend_last1,
                -away_reserves_3P_trend_last1,
                -away_reserves_3PA_trend_last1,
                -away_reserves_FT_trend_last1,
                -away_reserves_FTA_trend_last1,
                -away_reserves_ORB_trend_last1,
                -away_reserves_DRB_trend_last1,
                -away_reserves_TRB_trend_last1,
                -away_reserves_AST_trend_last1,
                -away_reserves_STL_trend_last1,
                -away_reserves_BLK_trend_last1,
                -away_reserves_TOV_trend_last1,
                -away_reserves_PF_trend_last1,
                -away_reserves_PTS_trend_last1,
                -away_team_Total_MP_trend_last1,
                -away_team_Total_FG_trend_last1,
                -away_team_Total_FGA_trend_last1,
                -away_team_Total_2P_trend_last1,
                -away_team_Total_2PA_trend_last1,
                -away_team_Total_3P_trend_last1,
                -away_team_Total_3PA_trend_last1,
                -away_team_Total_FT_trend_last1,
                -away_team_Total_FTA_trend_last1,
                -away_team_Total_ORB_trend_last1,
                -away_team_Total_DRB_trend_last1,
                -away_team_Total_TRB_trend_last1,
                -away_team_Total_AST_trend_last1,
                -away_team_Total_STL_trend_last1,
                -away_team_Total_BLK_trend_last1,
                -away_team_Total_TOV_trend_last1,
                -away_team_Total_PF_trend_last1,
                -away_team_Total_PTS_trend_last1,
                -away_opp_team_Total_MP_trend_last1,
                -away_opp_team_Total_FG_trend_last1,
                -away_opp_team_Total_FGA_trend_last1,
                -away_opp_team_Total_2P_trend_last1,
                -away_opp_team_Total_2PA_trend_last1,
                -away_opp_team_Total_3P_trend_last1,
                -away_opp_team_Total_3PA_trend_last1,
                -away_opp_team_Total_FT_trend_last1,
                -away_opp_team_Total_FTA_trend_last1,
                -away_opp_team_Total_ORB_trend_last1,
                -away_opp_team_Total_DRB_trend_last1,
                -away_opp_team_Total_TRB_trend_last1,
                -away_opp_team_Total_AST_trend_last1,
                -away_opp_team_Total_STL_trend_last1,
                -away_opp_team_Total_BLK_trend_last1,
                -away_opp_team_Total_TOV_trend_last1,
                -away_opp_team_Total_PF_trend_last1,
                -away_opp_team_Total_PTS_trend_last1,
                -away_reserves_Played_trend_last1,
                -away_opp_reserves_Played_trend_last1,
                -away_Pace_trend_last1,
                -away_team_Total_FG_pct_trend_last1,
                -away_team_Total_2P_pct_trend_last1,
                -away_team_Total_3P_pct_trend_last1,
                -away_team_TotalFT_pct_trend_last1,
                -away_team_Total_TmPoss_trend_last1,
                -away_opp_team_Total_TmPoss_trend_last1,
                -away_team_Total_ORATING_trend_last1,
                -away_team_Total_DRATING_trend_last1,
                -away_team_Total_EFG_pct_trend_last1,
                -away_team_Total_TS_pct_trend_last1,
                -away_team_Total_ORB_pct_trend_last1,
                -away_team_Total_DRB_pct_trend_last1,
                -away_team_Total_TRB_pct_trend_last1,
                -away_team_Total_HAST_pct_trend_last1,
                -away_team_Total_PAST_pct_trend_last1,
                -away_team_Total_AST_pct_trend_last1,
                -away_team_Total_STL_pct_trend_last1,
                -away_team_Total_BLK_pct_trend_last1,
                -away_team_Total_TOV_pct_trend_last1,
                -away_team_Total_AST_TO_trend_last1,
                -away_team_Total_FTr_trend_last1,
                -away_reserves_BDR_trend_last1,
                -away_starters_SLR_trend_last1,
                -away_team_Total_MP_avg_at_home,
                -away_team_Total_FG_avg_at_home,
                -away_team_Total_FGA_avg_at_home,
                -away_team_Total_2P_avg_at_home,
                -away_team_Total_2PA_avg_at_home,
                -away_team_Total_3P_avg_at_home,
                -away_team_Total_3PA_avg_at_home,
                -away_team_Total_FT_avg_at_home,
                -away_team_Total_FTA_avg_at_home,
                -away_team_Total_ORB_avg_at_home,
                -away_team_Total_DRB_avg_at_home,
                -away_team_Total_TRB_avg_at_home,
                -away_team_Total_AST_avg_at_home,
                -away_team_Total_STL_avg_at_home,
                -away_team_Total_BLK_avg_at_home,
                -away_team_Total_TOV_avg_at_home,
                -away_team_Total_PF_avg_at_home,
                -away_team_Total_PTS_avg_at_home,
                -away_Pace_avg_at_home,
                -away_reserves_Played_avg_at_home,
                -away_team_Total_FG_pct_at_home,
                -away_team_Total_2P_pct_at_home,
                -away_team_Total_3P_pct_at_home,
                -away_team_TotalFT_pct_at_home,
                -away_team_Total_EFG_pct_at_home,
                -away_team_Total_TS_pct_at_home,
                -away_team_Total_HAST_pct_avg_at_home,
                -away_team_Total_PAST_pct_at_home,
                -away_team_Total_TOV_pct_at_home,
                -away_team_Total_AST_TO_at_home,
                -away_team_Total_FTr_at_home,
                -home_team_Total_MP_avg_on_the_road,
                -home_team_Total_FG_avg_on_the_road,
                -home_team_Total_FGA_avg_on_the_road,
                -home_team_Total_2P_avg_on_the_road,
                -home_team_Total_2PA_avg_on_the_road,
                -home_team_Total_3P_avg_on_the_road,
                -home_team_Total_3PA_avg_on_the_road,
                -home_team_Total_FT_avg_on_the_road,
                -home_team_Total_FTA_avg_on_the_road,
                -home_team_Total_ORB_avg_on_the_road,
                -home_team_Total_DRB_avg_on_the_road,
                -home_team_Total_TRB_avg_on_the_road,
                -home_team_Total_AST_avg_on_the_road,
                -home_team_Total_STL_avg_on_the_road,
                -home_team_Total_BLK_avg_on_the_road,
                -home_team_Total_TOV_avg_on_the_road,
                -home_team_Total_PF_avg_on_the_road,
                -home_team_Total_PTS_avg_on_the_road,
                -home_Pace_avg_on_the_road,
                -home_reserves_Played_avg_on_the_road,
                -home_team_Total_FG_pct_on_the_road,
                -home_team_Total_2P_pct_on_the_road,
                -home_team_Total_3P_pct_on_the_road,
                -home_team_TotalFT_pct_on_the_road,
                -home_team_Total_EFG_pct_on_the_road,
                -home_team_Total_TS_pct_on_the_road,
                -home_team_Total_HAST_pct_avg_on_the_road,
                -home_team_Total_PAST_pct_on_the_road,
                -home_team_Total_TOV_pct_on_the_road,
                -home_team_Total_AST_TO_on_the_road,
                -home_team_Total_FTr_on_the_road)

df_vf$Total <- df_vf$home_Total_Score+df_vf$away_Total_Score

colnames(df_vf)[colnames(df_vf) %in% c("home_team_id")]<-c("HomeTeam")
colnames(df_vf)[colnames(df_vf) %in% c("home_Total_Score")]<-c("HomePts")
colnames(df_vf)[colnames(df_vf) %in% c("away_team_id")]<-c("VisitTeam")
colnames(df_vf)[colnames(df_vf) %in% c("away_Total_Score")]<-c("VisitPts")
colnames(df_vf)[colnames(df_vf) %in% c("date")]<-c("Date")
...
#   write_xlsx(file.path("output", "ncaab_var_names.xlsx"), col_names = FALSE)

write.csv(df_vf, "NCAAB_Final.csv", row.names = FALSE, na = "")
